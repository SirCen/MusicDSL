...
public class ITermsLowLevelAPI extends ITerms {
  ...
  public ITermsLowLevelAPI(int variableCount, int sequenceVariableCount) {
    this(variableCount, sequenceVariableCount, Set.of(ValueSymbols.values()));
  }

  public ITermsLowLevelAPI(int variableCount, int sequenceVariableCount, Set<ValueSymbols> specialSymbolsInUse) {
    super();
    ...
    for (ValueSymbols f : specialSymbolsInUse)
      findString(f.name());
  ...
  private Integer stringNextFreeIndex = 1; // defensive programming: index zero is never used
